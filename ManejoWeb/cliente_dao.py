...
class ClienteDAO:
    SELECCIONAR = 'SELECT * FROM clientes ORDER BY id_cliente'
    SELLECIONAR_ID='SELECT * FROM clientes WHERE id_cliente=%s'
    INSERTAR = 'INSERT INTO clientes(nombre_cliente, apellido_cliente, membresia_cliente) VALUES(%s, %s, %s)'
    ACTUALIZAR = 'UPDATE clientes SET nombre_cliente=%s, apellido_cliente=%s, membresia_cliente=%s WHERE id_cliente=%s'
    ...
    def seleccionar_id(cls,id):
        ...
        try:
            ...
            cursor = conexion.cursor()
            valores = (id,)
            cursor.execute(cls.SELLECIONAR_ID,valores)
